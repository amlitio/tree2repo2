...
export default function Tree2Repo() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-400 font-sans selection:bg-[#00FF85]/30 scroll-smooth relative overflow-x-hidden">
      <Head>
        <title>Tree2Repo | The Developer's Repository Scaffolder</title>
        <meta name="description" content="Instantly convert ASCII directory trees into downloadable repository packages. The ultimate developer utility for professional project scaffolding." />
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[#050505]/70 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <a href="#features" className="text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-[#00FF85] transition-colors">Features</a>
      ...
      <main className="relative z-10">
        ...
        <section className="px-6 pt-40 pb-32 text-center max-w-5xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#00FF85]/10 border border-[#00FF85]/20 text-[#00FF85] text-[10px] uppercase tracking-widest font-black mb-8">
            <Zap className="w-3.5 h-3.5 fill-current" /> Next-Gen Scaffolding
          </div>
          <h1 className="text-6xl md:text-9xl font-black text-white tracking-[calc(-0.04em)] leading-[0.85] mb-10">
            Visualize. <br />
          ...
          <p className="text-lg md:text-2xl text-zinc-500 max-w-3xl mx-auto mb-12 leading-relaxed font-medium">
            Stop the manual directory grind. Tree2Repo instantly converts visual ASCII diagrams into structured, ready-to-code local repositories.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-5">
            <a href="#tool" className="w-full sm:w-auto px-10 py-5 bg-[#00FF85] text-black font-black text-xl rounded-2xl shadow-[0_0_40px_rgba(0,255,133,0.3)] hover:shadow-[0_0_60px_rgba(0,255,133,0.5)] transition-all hover:-translate-y-1 inline-flex items-center justify-center gap-3">
              Start Building For Free <ArrowRight className="w-6 h-6 stroke-[3]" />
            </a>
            <a href="#features" className="w-full sm:w-auto px-10 py-5 bg-white/5 border border-white/10 text-white font-bold rounded-2xl hover:bg-white/10 transition-all">
              Technical Overview
            </a>
          </div>
        </section>
        ...
        <section id="tool" className="px-6 py-24 max-w-7xl mx-auto border-t border-white/5">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              <h2 className="text-3xl font-black text-white tracking-tight">Synthesizer V2</h2>
              <p className="text-zinc-500 text-sm mt-1">Configure your architecture in real-time.</p>
            </div>
            <div className="flex items-center gap-3 w-full md:w-auto bg-white/5 p-1 rounded-2xl border border-white/5">
               {['python', 'react', 'node'].map((t) => (
                 <button 
                  key={t}
                  onClick={() => setTreeText(TEMPLATES[t])}
                  className="flex-1 md:flex-none px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:text-white text-zinc-500"
                 >
                   {t}
                 </button>
               ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
            ...
            <div className="bg-[#0c0c0c] border border-white/10 rounded-[2.5rem] overflow-hidden backdrop-blur-3xl shadow-2xl flex flex-col group transition-all hover:border-[#00FF85]/30">
              <div className="px-8 py-5 border-b border-white/5 flex items-center justify-between">
                <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">Input Diagram</span>
                <div className="flex gap-1.5">
                  <div className="w-2.5 h-2.5 rounded-full bg-red-500/30"></div>
                  <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/30"></div>
                  <div className="w-2.5 h-2.5 rounded-full bg-[#00FF85]/30"></div>
                </div>
              </div>
              <textarea
                className="w-full h-[450px] lg:h-[550px] bg-transparent p-8 font-mono text-sm text-[#00FF85] outline-none resize-none leading-loose placeholder:text-zinc-800"
                value={treeText}
            ...
            <div className="bg-[#0c0c0c] border border-white/10 rounded-[2.5rem] overflow-hidden backdrop-blur-3xl shadow-2xl flex flex-col group transition-all hover:border-[#00FF85]/30 relative">
              <div className="px-8 py-5 border-b border-white/5">
                <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">Synthesis Engine Output</span>
              </div>
              <div className="p-8 h-[450px] lg:h-[550px] overflow-y-auto font-mono text-sm space-y-2.5 custom-scroll">
                {preview.map((item, i) => (
                  <div key={i} style={{ paddingLeft: `${item.depth * 28}px` }} className="flex items-center gap-4 transition-all hover:translate-x-1">
                    <span className="text-zinc-600 grayscale brightness-125 opacity-60 text-lg">
                      {item.isFolder? '📁' : '📄'}
                    </span>
                    <span className={item.isFolder? 'text-[#00FF85] font-black' : 'text-zinc-200 font-medium'}>
                      {item.name}
                    </span>
                  </div>
                ))}
        ...
        {/* ================= FEATURES SECTION ================= */}
        <section id="features" className="px-6 py-32 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
           <div className="p-10 bg-white/[0.02] border border-white/10 rounded-[2.5rem] space-y-6 hover:bg-white/[0.04] transition-all">
              ...
              <h3 className="text-white font-black text-xl">Advanced Parsing</h3>
           ...
           <div className="p-10 bg-white/[0.02] border border-white/10 rounded-[2.5rem] space-y-6 hover:bg-white/[0.04] transition-all">
              ...
              <h3 className="text-white font-black text-xl">Deep Nesting</h3>
              <p className="text-sm text-zinc-500 leading-relaxed font-medium">Our stack-based depth engine handles 20+ levels of directory complexity with zero path fragmentation.</p>
           </div>
           <div className="p-10 bg-white/[0.02] border border-white/10 rounded-[2.5rem] space-y-6 hover:bg-white/[0.04] transition-all">
              <Cpu className="text-[#00FF85] w-10 h-10" />
              <h3 className="text-white font-black text-xl">Client-Side Blob</h3>
              <p className="text-sm text-zinc-500 leading-relaxed font-medium">No server waiting. Synthesis happens entirely in your browser's RAM using JSZip for total privacy and speed.</p>
           </div>
        </section>
      </main>
      ...
      {/* ================= SUCCESS MODAL ================= */}
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z- p-4">
          <div className="bg-[#0a0a0a] border border-white/10 p-12 rounded-[3rem] max-w-md w-full text-center shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="w-24 h-24 bg-[#00FF85]/10 text-[#00FF85] rounded-full flex items-center justify-center mx-auto mb-8 border border-[#00FF85]/20">
              <CheckCircle2 className="w-12 h-12 stroke-[2.5]" />
            </div>
            <h3 className="text-3xl font-black text-white mb-3">Synthesis Ready</h3>
            <p className="text-zinc-500 font-medium mb-10 leading-relaxed">Your architecture has been processed. Check your <b>Downloads</b> folder for the completed structure.</p>
            <button onClick={() => setShowModal(false)} className="w-full py-5 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all">
              Close System
            </button>
          </div>
        </div>
      ...
      <footer className="px-6 py-16 border-t border-white/5 text-center text-[10px] text-zinc-700 font-black tracking-[0.4em] uppercase">
        &copy; 2026 Tree2Repo &bull; Optimized for Vercel &bull; Build 02.14
      </footer>
    </div>
  );
}